...
export default function School() {
  ...
  return (
    <div className="flex flex-col items-start self-stretch gap-[80px] pt-[60px] pr-[60px] pb-[60px] pl-[60px] w-full h-screen">
      <div className="flex justify-between items-center self-stretch">
        <div onClick={() => router.back()} className="cursor-pointer">
          <ArrowBackIosNewRoundedIcon className="text-[#FAFAFA]" />
        </div>
        <span className="flex w-full p-[10px] text-3xl text-zinc-50">
          {school?.schoolName}
        </span>
      </div>
        ...
        <div className="flex flex-col items-start gap-9 self-stretch">
          <div className="flex pt-[20px] pr-6 pb-5 pl-12 justify-between items-start self-stretch rounded-xl bg-[#2A3A50] bg-opacity-30">
            <div className="flex gap-12 items-center">
              <Image
                src={school?.pictureUrl || Profiler}
                width={100}
                height={100}
                alt="icon"
              ...
              <div className="flex flex-col items-start gap-4">
                <button className="flex justify-center items-center py-2 px-3 rounded-[6px] border border-[#2A3A50] text-zinc-50">
                  {school?.package[0].toUpperCase()}
                  {school?.package.slice(1).toLowerCase()}
                </button>
                <span className="flex w-full text-2xl text-zinc-50">
                  {school?.schoolName}
                </span>
                <span className="flex w-full text-xl text-zinc-50">
                  {school?.address} {school?.district} {school?.subDistrict}{" "}
                  {school?.province} {school?.postCode}
                </span>
                <div className="flex gap-4">
                  <span className="text-xl text-zinc-50">
                    ผู้สอน {school?.count.teacher} คน
                  </span>
                  <CircleIcon className="text-[#FAFAFA]" />
                  <span className="text-xl text-zinc-50">
                    ผู้เรียน {school?.count.student} คน
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex  items-start gap-9 w-full justify-between">
            ...
            <div className="flex flex-col items-start gap-9 w-full">
              <div className="flex flex-col items-start gap-6 self-stretch ">
                ...
                {teachers
                  ?.filter((teacher) => teacher.isEnable)
                  .map((teacher) => {
                    return (
                      <div
                        key={teacher.username}
                        className="flex items-center justify-between gap-2.5 self-stretch p-2 px-6"
                      >
                        <div className="flex items-center gap-5">
                          <Image
                            src={teacher.pictureUrl || Profiler}
                            alt="icon"
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <span className="text-zinc-50 text-base">
                              {teacher.email}
                            </span>
                          </div>
                        </div>
                        <div className="flex align-center gap-4">
                          <div
                            className={`flex align-center gap-4 py-2 px-4 rounded-md border border-[${statusActiveColor(teacher.isActived, teacher.allowLogin)}]`}
                          >
                            <CircleIcon
                              className={`${textStatusActiveColor(teacher.isActived, teacher.allowLogin)}`}
                            />
                            <span
                              className={`flex justify-center items-center text-[${statusActiveColor(teacher.isActived, teacher.allowLogin)}]`}
                            >
                              {textActivedUser(
                                teacher.isActived,
                                teacher.allowLogin,
                  ...
                  .map((student) => {
                    return (
                      ...
                      >
                        <div className="flex items-center gap-5">
                          <Image
                            src={student.pictureUrl || Profiler}
                            alt="icon"
                            className="w-16 h-16 rounded-full"
                            width={100}
                            height={100}
                        ...
                        <div className="flex align-center gap-4">
                          <div
                            className={`flex align-center gap-4 py-2 px-4 rounded-md border border-[${statusActiveColor(student.isActived, student.allowLogin)}]`}
                          >
                            <CircleIcon
                              className={`${textStatusActiveColor(student.isActived, student.allowLogin)}`}
                            />
                            <span
                              className={`flex justify-center items-center text-[${statusActiveColor(student.isActived, student.allowLogin)}]`}
                            >
                              {textActivedUser(
                                student.isActived,
                                student.allowLogin,
