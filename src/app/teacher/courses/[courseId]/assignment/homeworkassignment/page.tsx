...
export default function Assignment() {
  ...
  const [assignments, setAssignments] = useState<any[]>([]);  // ใช้เริ่มต้นเป็นอาเรย์ว่าง
  ...
  useEffect(() => {
    ...
    const fetchAssignments = async () => {
      ...
      try {
        ...
        if (data && data.data && data.data.assignment) {
          setAssignments(data.data.assignment); // ใช้ข้อมูลทั้งหมดที่ได้มา
        } else {
          setAssignments([]); // หากไม่พบข้อมูลให้ตั้งเป็นอาเรย์ว่าง
        }
        ...
        setAssignments([]);  // ตั้งค่าระหว่างเกิดข้อผิดพลาดเป็นอาเรย์ว่าง
      }
      setLoading(false);
    };
