...
const CourseUser: React.FC<CoursesRoleProps> = ({ data }) => {
  return <>
    <div className="flex justify-between py-1 items-center rounded-lg" key={data.username}>
      ...
      <div
        className={`flex text-white text-lg py-3 rounded-md border mx-6 w-36 space-x-2 text-center items-center justify-center ${data.isActived ? "border-green-l" : "border-white"
          }`}
      >
        <div
          className={`w-3 h-3 rounded-full ${data.isActived ? "bg-green-l" : "bg-white"
            }`}
        ></div>
