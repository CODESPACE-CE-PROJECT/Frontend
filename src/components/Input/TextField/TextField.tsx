...
export const TextField: React.FC<Props> = ({ placeholder, name, onChange, value, isNumberic, maxLength, validateText, isSubmited, textColor, bgColor, onKeyDown, require, className, disable}) => {
    ...
    return <>
        <input
            ...
            disabled={disable}
            className={`h-10 py-2 px-4 gap-2.5 w-full rounded-[6px] outline-0 disabled:text-gray-500 ${className} ${bgColor ? bgColor : 'bg-[#2A3A50]'} focus:outline-primary ${textColor ? textColor:'text-zinc-50'}`}
